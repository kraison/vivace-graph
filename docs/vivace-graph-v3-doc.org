...
#+BEGIN_SRC lisp
  ;; Nodes within RADIUS metres of a point, nearest first, as (node . distance):
  (find-nodes-near 49.20 37.17 500.0)

  ;; The K nodes nearest a point, nearest first (also (node . distance) pairs):
  (find-nearest-k 49.20 37.17 5)

  ;; Nodes whose location falls inside a :POLYGON / :MULTIPOLYGON area:
  (find-nodes-within my-task-area)
#+END_SRC
...
#+BEGIN_SRC lisp
  ...
  (select-flat (?o)
    (is-a ?o observation)
    (find-near ?o 49.20 37.17 500.0))

  ;; The 5 observations nearest (49.20, 37.17):
  (select-flat (?o)
    (is-a ?o observation)
    (find-nearest ?o 49.20 37.17 5))
...
~find-near/4~ takes ~?node lat lon radius-metres~; ~find-nearest/4~ takes ~?node lat lon k~; ~find-within/2~ takes ~?node area~.
...
- ~find-nodes-intersecting~ / ~find-intersects/2~ become **exact** (without GEOS they fall back to a coarse bounding-box overlap).
...
#+BEGIN_SRC lisp
  ...
  (find-nodes-intersecting my-task-area)
...
- ~node-geometry (node)~ ::
  Generic function (default ~nil~) returning the geometry a node occupies. Marking a geometry slot ~:index t~ wires its default up automatically; specialize it explicitly only for a *computed* geometry. Geometry-bearing nodes are then maintained automatically in the spatial index.

- ~find-nodes-near (lat lon radius &key graph)~ ::
  Live nodes within ~radius~ metres of ~(lat lon)~, nearest first, as ~(node . distance)~.

- ~find-nearest-k (lat lon k &key graph max-radius)~ ::
  The ~k~ nearest live nodes to ~(lat lon)~, nearest first, as ~(node . distance)~. Bounded by ~max-radius~ (default 25 km); returns fewer than ~k~ if fewer nodes lie within it.

- ~find-nodes-within (area &key graph)~ ::
  Live nodes whose location lies inside the ~:polygon~ / ~:multipolygon~ ~area~ geometry.

- ~(find-near ?node lat lon radius)~ / ~(find-nearest ?node lat lon k)~ / ~(find-within ?node area)~ ::
