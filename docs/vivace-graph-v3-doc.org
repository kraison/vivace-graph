...
#+BEGIN_QUOTE
*Known limitation (issue #56).* That repopulation is currently unreachable for
embeddings, because ~replay~ fails before it gets there. ~backup~ writes slot values
as Lisp text, and the restore readtable coerces *every* ~#(...)~ it reads to
~(simple-array (unsigned-byte 8) (*))~ — it has to, because node ~:ID~ values are byte
vectors and it cannot tell an id from slot data. A ~single-float~ vector therefore
signals a ~type-error~ on restore:

: The value 1.0 is not of type (UNSIGNED-BYTE 8)

So a graph holding ~:vector-index~ slots — or any slot with a non-byte vector — cannot
currently be round-tripped through ~snapshot~ and ~replay~. It fails loudly rather than
restoring corrupt data, but plan your backups accordingly: copy the graph *directory*
rather than relying on snapshot/replay until #56 is fixed.
#+END_QUOTE
