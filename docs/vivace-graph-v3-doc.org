...
- *Durable* — the constraint is backed by a *persistent* unique index (an on-disk skip list, or the in-RAM map on a memory-graph), reopened with the graph; it is not rebuilt by scanning every node on open.
...
| On-disk skip-lists for views and the spatial grid | In-RAM skip-lists (~mem-skip-list~) with the same cursor API |
...
- ~regenerate-all-views (graph)~ ::
  Force-rebuilds every view in the graph.  ~open-graph~ (and ~open-memory-graph~) accept ~:regenerate-views t~ to run this at open time.
