...
- *The index* is a skip list (in the graph's index region, ~indexes.dat~) keyed by fixed-precision *geohash* strings; each node id is stored under every grid cell its geometry occupies (duplicates allowed). A query computes the set of same-precision cells its window covers and looks each one up directly, so it touches only the cells in view. The grid precision is set with ~make-graph~'s ~:spatial-precision~ keyword (default 7, ≈150 m cells; 9 ≈ 5 m); it is persisted with the index and read back by ~open-graph~.
