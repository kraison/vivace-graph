...
stratum runs in **rounds**:

- **Round 0**: every rule in the stratum evaluated in full (`%desired`),
  its claims written in one transaction per rule in input order —
  kept when they stand, created when new. The round's **delta** is the
  new identities across the stratum, keyed by relation.
- **Round n+1**: for each rule and each of its recursive goals, one
  **variant** of the body in which that goal's `?c` is generated from
  the previous delta's claims of that relation and the rest of the body
  runs unchanged; the variants' solutions merge by dedupe key. Claims
  standing are kept, new identities created; the new ones are the next
  delta.
- **Termination**: a round with an empty delta. Identities are finite
...
union over rounds; after the last round, one pass per rule sweeps the
producer's claims outside that set and rewrites provenance for the kept
and new ones. Round writes are what let later rounds read earlier
derivations, since `claim/7` reads committed claims.

**Budget and refusals**: each round's evaluation is a `select` under
the run's resource bounds; a budget refusal, a commit constraint, or
the rounds cap stops the stratum with the previous derivation standing,
as a refused rule does today. **Cross-store scope** keeps the S3 shape:
each round's evaluation under `%under-snapshots`, the write in a
transaction on the own store; the not-serialised-against-a-later-premise
trap is per round now and documented as such.
